...
export default function Camera({ onVisitorCaptured = () => {} }) {
  ...
  const [isVerified, setIsVerified] = useState(false); // tracks if user has reviewed and submitted
            ...
            {Object.entries(visitorInfo).map(([key, value]) => (
              <div key={key} className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 borde text-gray-600 border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#5CA0AF] focus:border-[#5CA0AF] text-sm"
                />
              </div>
            ))}
